import { type Result, err, ok } from "npm:neverthrow";
 ...
 */
export function getAccount(customPrivateKey?: string): Result<PrivateKeyAccount, Error> {
  try {
    // 秘密鍵を取得
    ...
    if (customPrivateKey) {
      ...
      if (!envPrivateKey) {
        return err(new Error("Private key must be provided either as an argument or as PRIVATE_KEY environment variable"));
      }
